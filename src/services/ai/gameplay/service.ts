...
export const gameplayAiService = {
  ...
  async generateStoryTurn(
    ...
    try {
      ...
      let prefillContent = isNativeThinkingActive ? "" : (prefillAssistant || "");
      ...
      if (isNativeThinkingActive) {
        finalSystemPrompt += "\n\n<SYSTEM_OVERRIDE_NATIVE_THINKING>\n⚠️ NATIVE REASONING ACTIVE:\n- This model has native thinking enabled. Your thoughts are recorded natively by the system before generating text.\n- Do NOT output manual <thinking> or </thinking> tags in your response. Doing so will cause critical parsing errors.\n- Start your text response immediately with the <content> tag. Follow this strictly.\n</SYSTEM_OVERRIDE_NATIVE_THINKING>";
      }
  ...
  async auditLsrUpdates(
    ...
    try {
      ...
      const prompt = `You are an ultra-precise, razor-sharp Game State Auditor and Reality-Grounding Judge.
Your job is to read:
...
🚨 CRITICAL AUDITING & LORE LAWS (MUST OBEY UNCONDITIONALLY):
1. **STRICT ACQUISITION / LOOTING LAW**:
   - You MUST ONLY "add" or "update" an item in Table #6 (Túi đồ / Inventory) if the new story turn ("narrativeText") EXPLICITLY and CLEARLY states that the Main Character (MC) has successfully picked up, looted, purchased, received, or stored that item.
   - **NO BACKGROUND ITEMS**: If the story merely describes the environment, surroundings, or an item lying on the ground (e.g., "A gleaming sword rests on the altar", "Sophia notices a glowing herb under the tree", "The campfire is burning"), you MUST NOT add this item to the MC's inventory. The MC MUST actively pick it up first.
   - **NO NPC OWNERSHIP LEAKAGE**: Items currently held or owned by other NPCs must NOT be added to the MC's inventory.
   - **NO GENESIS CHEATS**: Do not assume the MC received an item unless it's explicitly narrated in the text.

2. **ANTI-GENRE DRIFT & COHERENCE LOCK**:
...
- action: "delete" if removing a record (e.g. character dies, item is fully consumed/destroyed/lost, matching rowKey = column index 0).
